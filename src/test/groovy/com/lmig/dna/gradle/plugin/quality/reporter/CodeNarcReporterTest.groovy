...
class CodeNarcReporterTest extends AbstractKitTest {
	...
	def "Check codenarc report"() {
		...
		build("""
            ...
            task testReport() {
				doLast {
                	new com.lmig.dna.gradle.plugin.quality.report.CodeNarcReporter().report(project, 'main')
				}
            }
        """)
		file('src/main/groovy').mkdirs()
		fileFromClasspath('build/reports/codenarc/main.xml', '/com/lmig/dna/gradle/plugin/quality/report/codenarc/main.xml')
